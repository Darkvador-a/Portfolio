...
class Model_Mapper_Web
{
    ...
    public function rowToObject($row)
    {
        ...
        $web->setTitle($row[Model_DbTable_Web::COL_TITLE]);
        $web->setUrl($row[Model_DbTable_Web::COL_URL]);
        $web->setFeature($row[Model_DbTable_Web::COL_FEATURE]);
        $web->setDescription($row[Model_DbTable_Web::COL_DESCRIPTION]);
        $web->setTechnology($row[Model_DbTable_Web::COL_TECHNOLOGY]);
        $web->setUpload($row[Model_DbTable_Web::COL_UPLOAD]);
